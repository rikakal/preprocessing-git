...
# ESPG 3857 uses meters as its unit of measurement
# 200 ft = 60.96 meters
    ...
    for i, row in tqdm(p.iterrows(), total=p.shape[0], desc='Cleaning'):
        if row['NEIGHBORS']:
            ...
            for neighbor in n_list:
                length = geo.intersection(
                    p.loc[int(neighbor)]['geometry']).length
                if length != 0:
...
start_time = time.time()
columns_read = [col for col in block_group_file.columns
           if col.startswith('P002') or col.startswith('P004')
           or col == 'geometry']
block_group = block_group_file[columns_read].copy()
block_to_precinct = maup.assign(block_group, precinct)
columns_sum = [col for col in block_group.columns
           if col.startswith('P002') or col.startswith('P004')]
summed_columns = block_group[columns_sum].groupby(block_to_precinct).sum()
